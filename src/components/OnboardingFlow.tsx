  ...
  const handleNext = () => {
    ...
    if (step === 1) {
      ...
      if (isNaN(parsedAge) || parsedAge < 15 || parsedAge > 35) {
        setErrorStatus('Age verification required. Please enter a valid age between 15 and 35 to continue.');
        return;
      }
      ...
      >
          ...
          <div className="flex items-center gap-2">
            ...
            {onCancel && (
              <button 
                type="button"
                onClick={onCancel}
                className="p-1 px-2 hover:bg-slate-250 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors cursor-pointer text-[10px] font-black uppercase tracking-wider flex items-center gap-1 border border-slate-200/50 dark:border-slate-800"
        ...
        <AnimatePresence mode="wait">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select 
                      ...
                      className="input-field py-2.5 px-3 text-xs font-semibold w-full bg-white border border-slate-200"
                    >
              ...
              <div className="flex gap-3">
                {onCancel && (
                  <button 
                    type="button" 
                    onClick={onCancel}
                    className="w-1/3 py-3.5 rounded-2xl border border-slate-200 hover:bg-slate-50 text-slate-500 font-black text-[10px] uppercase tracking-widest transition-colors cursor-pointer"
                  >
                    Exit Setup
                  </button>
                )}
          ...
          {step === 2 && (
            ...
            >
              <div>
                <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight">Daily Alerts & Consistency</h3>
              ...
              <div className="p-5 bg-slate-50 border border-slate-100 rounded-3xl space-y-4">
                <div className="flex justify-between items-center">
                  <div>
                    <h4 className="text-xs font-black uppercase text-slate-900">Activate daily alert</h4>
              ...
              <div className="flex gap-3">
                <button 
                  type="button" 
                  onClick={() => setStep(1)}
                  className="w-1/3 py-3 rounded-2xl border border-slate-200 hover:bg-slate-50 text-slate-500 font-black text-[10px] uppercase tracking-widest transition-colors"
                >
                  Back
                </button>
                <button 
            ...
            >
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button 
                  ...
                  className="btn-primary flex-1 py-3 text-xs font-black uppercase tracking-widest shadow-xl flex items-center justify-center gap-2"
                >
                  Review Privacy <ChevronRight size={14} />
                </button>
              </div>
            </motion.div>
          )}

          {/* STEP 4: System Disclaimer & Data Privacy Transparency */}
          {step === 4 && (
            <motion.div 
              key="step4"
              initial={{ opacity: 0, x: 10 }}
            ...
            >
              <div className="text-center">
                <div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center mx-auto mb-3 shadow-xs border border-emerald-100/60">
                  <ShieldCheck size={24} />
                </div>
                <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight">
                  {t.privacyScreen}
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 space-y-3.5 text-slate-600 text-xs leading-normal">
                <div className="flex gap-2 items-start">
                  <Lock size={16} className="text-blue-600 shrink-0 mt-0.5" />
                  <p className="font-semibold text-[11px]">
                    <span className="text-slate-900 font-black">Secure Encryption:</span> All reported moods, notes, and medical markers are stored on encrypted Cloud Firestore tables or strictly sandbox-contained local storages.
                  </p>
                </div>
                <div className="flex gap-2 items-start">
                  <Eye className="text-indigo-600 shrink-0 mt-0.5" size={16} />
                  <p className="font-semibold text-[11px]">
                    <span className="text-slate-900 font-black">No Commercial Sharing:</span> Your reflections and clinical metrics are strictly confidential. We promise <span className="font-black text-slate-800">100% advertising separation</span>—your information is NEVER sold.
                  </p>
                </div>
                <div className="flex gap-2 items-start">
                  <ShieldAlert className="text-amber-600 shrink-0 mt-0.5" size={16} />
                  <p className="font-semibold text-[11px] italic text-amber-900">
                    <span className="text-slate-900 font-black not-italic">Health Disclaimer:</span> VitalMind provides self-awareness trends and automated guidelines. It does not replace medical diagnostics or clinical interventions.
                  </p>
                </div>
              </div>

              <div className="p-4 bg-amber-50/50 border border-amber-100 rounded-2xl text-[10px] text-amber-800 text-center italic font-semibold">
